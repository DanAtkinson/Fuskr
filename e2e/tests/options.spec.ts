...
testWithExtension.describe('Options page', () => {
  ...
  testWithExtension(
    ...
    async ({ extensionContext: { context, extensionId } }) => {
      ...
      await expect(fitOnPageRadio).toBeVisible({ timeout: 5_000 });
      ...
      await expect(overloadToggle).toBeVisible({ timeout: 5_000 });
  ...
  testWithExtension(
    ...
    async ({ extensionContext: { context, extensionId } }) => {
      ...
      await expect(toggleBrokenImages).toBeVisible({ timeout: 5_000 });
      ...
      const statusMessage = page.locator('.status-message, [class*="status"]').filter({ hasText: /saved|success/i });
      const statusVisible = await statusMessage.isVisible({ timeout: 3_000 }).catch(() => false);
  ...
  testWithExtension(
    ...
    async ({ extensionContext: { context, extensionId } }) => {
      ...
      await expect(displayModeRadios).toHaveCount(4, { timeout: 5_000 });

      // Click the "thumbnails" radio
      const thumbnailsRadio = page.locator('#thumbnails');
      await expect(thumbnailsRadio).toBeVisible({ timeout: 5_000 });
      await thumbnailsRadio.click();

      // The thumbnails radio should now be checked
      await expect(thumbnailsRadio).toBeChecked({ timeout: 3_000 });
